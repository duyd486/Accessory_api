...
public sealed class ManageUserController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<object>>> CreateAdmin([FromBody] CreateStaffRequest request)
    {
        ...
        try
        {
            ...
            return Ok(ApiResponse<object>.Ok(new { user = dto }, "Tạo admin thành công."));
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult<ApiResponse<object>>> CreateStaff([FromBody] CreateStaffRequest request)
    {
        if (!IsAdmin())
        {
            return StatusCode(StatusCodes.Status403Forbidden, ApiResponse<object>.Fail("Chỉ admin mới có quyền tạo nhân viên!"));
        }
        ...
        try
        {
            ...
            return Ok(ApiResponse<object>.Ok(new { user = dto }, "Tạo nhân viên thành công."));
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, ApiResponse<object>.Fail(ex.Message));
